...
def web_driver_get(url):
    driver.get(url)

def load_login():
    web_driver_get(libraryLoginPage)
    input('Press ENTER once logged in...')
    driver.maximize_window()

def load_book_page(page_id):
    web_driver_get(f'https://{libraryMosaic}/reader/books/{args.isbn}/pageid/{page_id}')
...
def book_scrape_metadata():
    ...
    for i in range(5):
        ...
        with open(f'{args.output}/{args.isbn}-book_info.txt', 'wb') as book_file:
            pickle.dump(book_info, book_file)
        break

    if args.only_scrape_metadata:
        #print(len(book_info['pages']))
        #print(book_info['pages'][len(book_info['pages'])-1]['page'])
        driver.close()
        del driver

def book_scrape_pages():
    if os.path.exists(f'{args.output}/{args.isbn}-book_info.txt'):
        with open(f'{args.output}/{args.isbn}-info.txt', 'rb') as book_info_file:
            book_info = pickle.load(book_info_file)
            total_pages = len(book_info['pages'])
    else:
    ...
    if args.end_page > 0:
        ...
        pages_to_be_scraped = args.end_page - args.start_page + 1
    else:
        print(f'End   page: {total_pages}')
        pages_to_be_scraped = total_pages - args.start_page + 1
    
    print('Pages to be scraped:', pages_to_be_scraped)
    ...
    if not os.path.exists(f'{args.output}/{args.isbn}-page_urls.txt'):
        ...
        page_urls = set()
        failed_pages = set()
        actual_page = args.start_page
        bar = tqdm(total=pages_to_be_scraped)
        bar.update(actual_page)
        while actual_page < total_pages + 1:
            ...
            retry_delay = 5
            base_url = None
            for i in range(3):  # retry the page max this many times
                for request in driver.requests:
                ...
                if base_url:
                    break
                bar.write(f'Could not find a matching image for page {actual_page}, sleeping {retry_delay}s...')
                time.sleep(retry_delay)
                retry_delay += 5
            if not base_url:
                bar.write(f'Failed to get a URL for page {actual_page}, retrying later.')
                failed_pages.add(actual_page)
            else:
            ...
            if actual_page == args.end_page:
                bar.write(f'Exiting on page {actual_page}.')
                break

            # On the first page the back arrow is disabled and will trigger this
            if isinstance(actual_page, int) and actual_page > 0:
                try:
...
def main():
    global args, book_info, ebook_files, non_number_pages, front_pages, libraryLoginPage, libraryMosaic, libraryJigsaw
    ...
    if args.end_page > 0 and args.end_page < args.start_page:
        raise SyntaxError('End page cannot be lower than start page')
    ...
    #######################################
    # Create a ordered array of files considering frontcover pages, roman numbered pages and arabic numbered paged
    ...
    for file in pageFilesArr:
        ...
        if file_name.isdigit():
            ...
            # If the name is a word (like "Front"), consider it as 1
            existing_numbers.append(1)
        else:
            # If not number or word, try converting from roman to arabic
            try:
                arabic_number = roman.fromRoman(file_name.upper())
                existing_numbers.append(arabic_number)
            except roman.InvalidRomanNumeralError:
                # If not possivel converting, ignore it
                continue
    ...
    if not args.skip_ocr:
        print('Running OCR...')
        subprocess.run(f'ocrmypdf -l {args.language} --title "{title}" --jobs $(nproc) "{pdfRawFilePath}" "{tempfilePath}"', shell=True)
    else:
        print('Skipping OCR...')
